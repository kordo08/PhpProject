...
$port = "5432";
 ...
 if(isset($_POST)){
  ...
  $req1=$db->query("INSERT INTO niveaux VALUES ('' ,1,true,false,0.0,'.$id_user.'),
   ...
   $rcp_id1 = $db->query('SELECT id_niveau FROM niveaux WHERE num_niveau = 1 && id_user= "'.$id_user.'"');
    ...
    //insertion des theme dans la table theme
            ...
            {
                 if($i==0){
                       $req211=$db->query("INSERT INTO themes VALUES ('' ,'signalisation',true,0.0,'url_vedio','.$id_niv.')");
                          }
                       else{
                        $req211=$db->query("INSERT INTO themes VALUES ('' ,'signalisation',false,0.0,'url_vedio','.$id_niv.')");
                          }
           
              $req222=$db->query("INSERT INTO themes VALUES ('' ,'circulation',false,0.0,'url_vedio','.$id_niv.'),
                                                            ('' ,'divers',false,0.0,'url_vedio','.$id_niv.');");
                                                        
               $id_niv=$id_niv+1;
            }
       
        $rcp_id2 = $db->query('SELECT id_theme FROM themes WHERE titre_theme = "signalisation" && id_niveau ="'.$id_niveau.'"');
              $row2 = $rcp_id2->fetch();
               $id_th=$row2['id_theme'];
               $id_theme=$id_th;
 ...
 //***********************************************insertion des cours
   //theme1
 $req3=$db->query("INSERT INTO cours VALUES ('' ,'danger',true,0,'.$id_th.'),
                                            ('' ,'intersection',false,0,'.$id_th.');");
              
 //theme2
$id_th=$id_th+1;

$req4=$db->query("INSERT INTO cours VALUES ('' ,'sans panneau-partie1',false,0,'.$id_th.'),
 ...
 //theme3
 $id_th=$id_th+1;
$req5=$db->query("INSERT INTO cours VALUES ('' ,'Categorie B',false,0,'.$id_th.'),
...
$id_th=$id_th+1;
 ...
 $re6=$db->query("INSERT INTO cours VALUES ('' ,'obligation',false,0,'.$id_th.'),
                                           ('' ,'interdiction',false,0,'.$id_th.'); ");
                                                                                                        
                
//theme5
$id_th=$id_th+1;
$req7=$db->query("INSERT INTO cours VALUES ('' ,'avec panneau-partie1',false,0,'.$id_th.'),
                                           ('' ,'avec panneau-partie2',false,0,'.$id_th.');");
                
//theme6
$id_th=$id_th+1;
 $req8=$db->query("INSERT INTO cours VALUES ('' ,'La vitesse',false,0,'.$id_th.'),
                                            ('' ,'Les distances de sécurité',false,0,'.$id_th.'),
                                            ('' ,'L\'arrêt et le stationnement',false,0,'.$id_th.');  ");
                                          
 //theme7
 $id_th=$id_th+1;
                $req9=$db->query("INSERT INTO cours VALUES ('' ,'indication',false,0,'.$id_th.'),
                                                           ('' ,'temporaire',false,0,'.$id_th.');");
               
//theme8
$id_th=$id_th+1;
$req10=$db->query("INSERT INTO cours VALUES ('' ,'carrfours à sens giratoire',false,0,'.$id_th.')");
//theme9
 $id_th=$id_th+1;
$req11=$db->query("INSERT INTO cours VALUES ('' ,'Les dépassements',false,0,'.$id_th.'),
                                            ('' ,'La conduite sur autoroute',false,0,'.$id_th.'),
                                           ('' ,'Conduite en conditions difficiles',false,0,'.$id_th.') ;");
                                         
               
                
 $rcp_id3 = $db->query('SELECT id_cours FROM cours WHERE titre_cours = "danger" && id_theme = "'.$id_theme.'"');
$row3 = $rcp_id3->fetch();
 $id_co=$row3['id_cours'];
               
  $id_cours_qst=$id_co;
        // danger
$req12=$db->query("INSERT INTO questions VALUES ('' ,'multi','L\'usager de la route doit faire attention à un danger :'
 ...
 $id_co=$id_co+1;
 // intersections
 $req13=$db->query("INSERT INTO questions VALUES ('' ,'multi','Ce panneau annonce:' ,'pit_1','Une route à grande circulation'
                                                   ,'.$id_co.'),
  ...
  $id_co=$id_co+1;
        // intersection_cours niv1 lecon1  
            $req14=$db->query("INSERT INTO questions VALUES ('' ,'multi','Parmis ces différents choix indiquer le plus approprié pour eviter le risque d\'accident dans le circuit suivant :' ,'exo3_01','2/1','.$id_co.'),
      ...
      $id_co=$id_co+1;
        ...
        $req15=$db->query("INSERT INTO questions VALUES ('' ,'multi','Parmis ces différents choix indiquer le plus approprié pour eviter le risque d\'accident dans le circuit suivant :' ,'exo41_01','1/2','.$id_co.'),
       ...
       $req16=$db->query("INSERT INTO questions VALUES ('' ,'multi','Quelle est la durée de validité du Permis B ?','','10 ans','.$id_co.'),
...
('' ,'vrai/faux','Je limite ma vitesse à 80 K/h pendant : 1ans','','faux', '.$id_co.')
         ...
         $id_co=$id_co+1;
        // _cours niv1 diver2
       $req17=$db->query("INSERT INTO questions VALUES ('' ,'multi','S\'il pleut la journée je dois allumer les feux de :'
...
('' ,'vrai/faux','Les feux de position seuls sont suffisants en cas d\'intempéries:',
 ...
 ('' ,'vrai/faux',' L\’utilisation des feux de route en ville ou des feux de brouillard arrière par temps de pluie, puissent représenter un usage abusif des feux',
         ...
         $id_co=$id_co+1;
        // _cours niv1 diver3
       $req18=$db->query("INSERT INTO questions VALUES
...
('' ,'vrai/faux','Un véhicule de police en intervention est : prioritaire','','vrai', '.$id_co.')
         ...
         $id_co=$id_co+1;
                // _cours  obligation 
       $req20=$db->query("INSERT INTO questions VALUES ('' ,'multi','Que signifie ce panneau ?' ,'po_1','Direction obligatoire
...
('' ,'multi','Il s\'agit d\'une obligation d\'aller:' ,'po_9', 'Tout droit ou à gauche à la prochaine intersection','.$id_co.'),
        ('' ,'multi','Il s\'agit d\'une obligation d\'aller:','po_10' ,'Tout droit ou à droite à la prochaine intersection','.$id_co.'),
         ...
         $id_co=$id_co+1;
        // _cours interdiction
       $req19=$db->query("INSERT INTO questions VALUES ('' ,'multi','Ce panneau signifie qu\'il est interdit:','pir_11',
       ...
       $req21=$db->query("INSERT INTO questions VALUES ('' ,'multi','Parmis ces différents choix indiquer le plus
       ...
       $req22=$db->query("INSERT INTO questions VALUES ('' ,'multi','Parmis ces différents choix indiquer le plus
              ...
              $id_co=$id_co+1;
        // _cours niv2 diver1
       $req23=$db->query("INSERT INTO questions VALUES ('' ,'multi','Quelle est l\'augmentation de consommation à 120km/h si on a une galerie sur le toit ?' ,'','10%','.$id_co.'),
...
('' ,'vrai/faux','Les vitesses maximales autorisée dans les routes en générale : 100km (chaussée humide :80 km/h)','','vrai', '.$id_co.')
         ...
         $id_co=$id_co+1;
        // _cours niv2 diver2
       $req24=$db->query("INSERT INTO questions VALUES ('' ,'multi','La distance de sécurité quand on roule à 70 km/h est :' ,'',
                                                       '42 m','.$id_co.'),
...
('' ,'vrai/faux','La distance de sécurité est d\’autant plus grande lorsque la vitesse est élevée','','vrai', '.$id_co.')
         ...
         $id_co=$id_co+1;
        // _cours niv2 diver3
  $req25=$db->query("INSERT INTO questions VALUES('' ,'multi','Parmi ces cas lesquelles sont des cas d\'arrêt et de stationnement dangereux :'
...
('' ,'vrai/faux','Ligne jaune discontinue sur la chaussée ou le trottoir : arrêt et stationnement interdit','','faux', '.$id_co.')
         ...
         $id_co=$id_co+1;
        // _cours indication
       $req26=$db->query("INSERT INTO questions VALUES('' ,'multi','Ce panneau indique:' ,'pin_7',
         ...
         $id_co=$id_co+1;
        // _cours  temporaire 
       $req27=$db->query("INSERT INTO questions VALUES ('' ,'multi','L\'usager de la route doit faire attention à un danger temporaire :' ,'pt_1','Chaussée rétrécie temporaire','.$id_co.'),
       ...
       $req28=$db->query("INSERT INTO questions VALUES ('' ,'multi','Parmis ces différents choix indiquer le plus
        ...
        $id_co=$id_co+1;
        // _cours niv3 diver1
       $req29=$db->query("INSERT INTO questions VALUES ('' ,'multi','Si je dépasse un vélo en ville la distance minimale à respecter est de :' ,'','1 m','.$id_co.'),
...
('' ,'vrai/faux','Si je dépasse un piéton en dehors de la ville la distance minimale à respecter est de: 1,5 m','','vrai', '.$id_co.')
                ...
                $id_co=$id_co+1;
        // _cours niv3 diver2
       $req30=$db->query("INSERT INTO questions VALUES ('' ,'multi','En circulation normale je dois être :' ,'',
...
('' ,'vrai/faux','On ne doit jamais dépasser par la droite un usager qui s’entête à rouler sur une voie située à gauche','','vrai', '.$id_co.'),
 ('' ,'vrai/faux','Il y a du brouillard sur l\'autoroute et que je ne vois pas à 50 mètres je dois circuler au maximum à : 50 km/h','','vrai', '.$id_co.')
         ...
         $id_co=$id_co+1;
        // _cours niv3 diver3
       $req31=$db->query("INSERT INTO questions VALUES  ('' ,'multi','Si je circule sur une zone exposée au vent :'
