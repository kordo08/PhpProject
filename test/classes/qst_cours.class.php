...
public function table_final(array $tab1,array $tab2,int $nbQstParType,string $titre_theme,string $titre_cours)
{
    ...
    while ($i<2*$nbQstParType)
    {
        ...
        if (!in_array($indice, $tableau2)) {
            ...
            {
            $tab_choix=$this->get_choices($titre_theme,'','',$tab2[$indice]['contenu']);
            }
            ...
            {
            $tab_choix=$this->get_choices($titre_theme,$titre_cours,$tab2[$indice]['image_quiz'],'');   
            }
            if((strcasecmp($titre_theme,'circulation')==0) && strlen($tab2[$indice]['reponse'])==3)
            {
            $tabQst[$i]['choix_1']=$tab_choix[0];
            $tabQst[$i]['choix_2']="les deux en meme temps";   
            }
           else
           {
            $tabQst[$i]['choix_1']=$tab_choix[0];
...
public function get_choices(string $titre_theme,string $titre_cours,string $reponse_juste,string $qst)
{
 if (strcasecmp($titre_theme,'signalisation')==0)
 {
 ...
 if(strcasecmp($titre_theme,'divers')==0)
 {
    $sql="SELECT * FROM choix_divers WHERE qst=?";
    $stmt=$this->connect()->prepare($sql);
    $stmt->execute([$qst]);
    $tab=$stmt->fetchAll(); 
